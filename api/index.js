...
// Always respond with JSON for errors — never let Express send HTML error pages
...
// Root — API info (always JSON on Vercel, dashboard on local)
app.get('/', (req, res) => {
  if (IS_VERCEL) {
    return res.json({
      status: 'ok',
      ...
      endpoints: {
        ...
        stats:    'GET  /api/stats',
        solve:    'GET  /turnstile?url=TARGET_URL&sitekey=SITE_KEY',
        gologin:  'POST /api/go',
        proxies:  'GET  /api/proxies/download',
        accounts: 'GET  /api/accounts/download',
      },
      usage_example: '/turnstile?url=https://example.com&sitekey=0x4AAAAAAA...',
    });
  }
...
app.get('/turnstile', handleSolve);
app.post('/turnstile', handleSolve);
